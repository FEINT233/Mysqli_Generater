...
require_once BASE_PATH."Sql/example/12345Example.class.php";
class 12345Dao extends SqlDaoInterface{
    private $conn;
    private $tableName = "12345";
    /**
     * 获取数据库连接,每次进行数据库操作后关闭数据库连接
     */
    private function getConn(){
        $MysqlConnect = new MysqlConnect();
        $this->conn = $MysqlConnect->getMysqlConnect_Mysqli("123");
    }
    /**
     * 全字段查询,返回一个数组
     * 数组里有对象信息的键值对格式信息
    ...
    public function selectByExample($POJOExample){
        if($POJOExample instanceof 12345Example){
            $this->getConn();
            $values = "";
