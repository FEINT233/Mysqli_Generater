...
require_once BASE_PATH."Sql/example/StudentExample.class.php";
class StudentDao extends SqlDaoInterface{
    private $conn;
    ...
    public function select(){
        ...
        while($row=$result->fetch_array()){
            $student = new Student();
            $student->setStuId($row["stu_id"]);
            $student->setStuName($row["stu_name"]);
            $student->setStuSex($row["stu_sex"]);
    ...
    public function selectByExample($POJOExample){
        if($POJOExample instanceof StudentExample){
            $this->getConn();
            $values = "";
    ...
    public function insert($POJO){
        if($POJO instanceof Student){
            $this->getConn();
            $values = "";
            $values .= "'".$POJO->getStuId()."',";
            $values .= "'".$POJO->getStuName()."',";
            $values .= "'".$POJO->getStuSex()."',";
    ...
    public function insertByExample($POJO){
        if($POJO instanceof Student){
            ...
            $keys = "";
            $values = "";
            if($POJO->getStuId() != null){
                $keys .= "stu_id,";
                $values .= "'".$POJO->getStuId()."',";
            }
            if($POJO->getStuName() != null){
                $keys .= "stu_name,";
                $values .= "'".$POJO->getStuName()."',";
            }
            if($POJO->getStuSex() != null){
                $keys .= "stu_sex,";
                $values .= "'".$POJO->getStuSex()."',";
            }
    ...
    public function updateByPK($POJO){
        if($POJO instanceof Student){
            $this->getConn();
            $id = "";
            $id = "stu_id = '$POJO->getStuId()'";
            $values = "";
            if($POJO->getStuId() != null){
                $values .= "stu_id='".$POJO->getStuId()."',";
            }
            if($POJO->getStuName() != null){
                $values .= "stu_name='".$POJO->getStuName()."',";
            }
            if($POJO->getStuSex() != null){
                $values .= "stu_sex='".$POJO->getStuSex()."',";
            }
